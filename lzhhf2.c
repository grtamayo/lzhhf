/*
	...
	Filename:      lzhhf2.c  (eXtractor:  lzhhfx2.c)
	Written by:    Gerald Tamayo, Oct. 22, 2008 (2/24/2022)(2/16/2023)
...
lists which contain the various occurrences or "positions" of
particular strings in the sliding-window that hashes to the same 
value. The array is indexed by the various string hashes.
...
dpos_t search( unsigned char *w, unsigned char *p )
{
	int j, winpos, len, m = 0, n = 0;
	dpos_t dpos = { 0, 0 };
	
	/* point to start of lzhash[ index ] */
	winpos = lzhash[ hash(p,pat_cnt,pat_MASK,win_MASK) ];
	
	if ( buf_cnt > 1 ) while ( winpos != LZ_NULL ) {
		/* ---- FAST LZ77 SEARCH ----
		...
		*/
		...
		len = dpos.len+1;
		if ( len < buf_cnt )
			while ( p[ j++ & pat_MASK ] == w[ (winpos+len) & win_MASK ]
				&& (++len) < buf_cnt ) ;
...
void put_codes( dpos_t *dpos )
{
	...
	if ( dpos->len >= MIN_LEN ) {
		k = dpos->pos;
		/* dynamically encode the MSByte via FGK. */
		fgk_encode_symbol( mtf(k >> hash_SHIFT) );
		put_nbits( k, hash_SHIFT );
    }
    else {
		dpos->len = 1;
		/* emit just the byte. */
